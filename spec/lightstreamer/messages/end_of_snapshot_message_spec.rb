describe Lightstreamer::EndOfSnapshotMessage do
  ...
  it 'parses valid end-of-snapshot messages' do
    expect(Lightstreamer::EndOfSnapshotMessage.parse('18,1,EOS', 18, items).item_index).to eq(0)
    expect(Lightstreamer::EndOfSnapshotMessage.parse('18,10,EOS', 18, items).item_index).to eq(9)
  end

  it 'rejects invalid end-of-snapshot messages' do
    ['', '1,1,EO', '18,20,EOS', '18,10,EOS '].each do |line|
      expect(Lightstreamer::EndOfSnapshotMessage.parse(line, 18, items)).to be nil
    end
  end
end
